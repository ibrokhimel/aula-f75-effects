...
import {
  REACTIVE, REACTIVE_CATEGORIES, WINDOW, LED_FOR_CODE, makePress,
...
export function ReactivePanel({ device, log }: Props) {
  ...
  useEffect(() => {
    ...
    const down = (e: KeyboardEvent) => {
      ...
      const p = makePress(led, clock(), seqRef.current++);
  ...
  const renderFrame = useCallback<AnimationFn>(() => {
    ...
    // frame anyway, and held keys must survive regardless of age.
    pressesRef.current = pressesRef.current.filter(
      (p) => p.release === null || t - p.release < WINDOW,
    );
    return def.fn(t, pressesRef.current);
  }, []);
